...
> Everything on this page is specific to the **Cursor-native implementation**
> in `.cursor/`. If you're using Codex, Claude Code, or Windsurf, see
> [`lambda-engine/CORE.md`](../lambda-engine/CORE.md) and
> [`docs/architecture.md`](architecture.md) instead — the underlying
> architecture is the same, but these slash commands don't exist outside
> Cursor. For the exhaustive parameter-by-parameter command reference, see
> [`.cursor/tuts/COMMANDS.md`](../.cursor/tuts/COMMANDS.md).
...
- **60+ Rules**: Comprehensive rule system with general, workflow, meta, and service rules
- **15+ Hooks**: Automation hooks for documentation, security, compliance, and validation
...
See [`docs/architecture.md`](architecture.md) for the full operator vocabulary
...
2. **Rules**: Add `.mdc` files to the appropriate category in `.cursor/rules/`
3. **Update**: Update `.cursor/tuts/COMMANDS.md` for new commands

For repo-wide contribution guidelines (including the Claude Code skills and
slash commands in `.claude/`), see the [Contributing](../README.md#contributing)
